...
def detect_multiple_texts_in_video(video_path, target_texts, frame_interval=10, display_duration=2000, ignore_range=5, display_scale=0.5):
    ...
    #for frame_number in tqdm(range(0, total_frames), desc="Processing frames", unit="frames"):
        ...
        for target_text in target_texts:
            ...
            if re.search(rf'\b{re.escape(target_text.lower())}\b', text):
    ...
    # Execute a command for each detected text
    for target_text, timestamps in timestamps_dict.items():
        if timestamps:
            for timestamp in timestamps:
                # Replace the following command with your desired command
                print(f"ffmpeg -y -ss {int(timestamp)-5} -i \"{video_path}\" -t 10 -acodec copy -vcodec copy -avoid_negative_ts 1 \"{output_folder}\\{int(timestamp)}-{target_text}.mp4\"")			
                command = f"ffmpeg -y -ss {int(timestamp)-5} -i \"{video_path}\" -t 10 -acodec copy -vcodec copy -avoid_negative_ts 1 \"{output_folder}\\{int(timestamp)}-{target_text}.mp4\""
    ...
    print(f" Started at {start_time} & Ended at {end_time} Total Process Duration: {total_time:.2f} seconds")
...
timestamps_dict = detect_multiple_texts_in_video(video_path, target_texts, frame_interval=60, display_duration=1, ignore_range=5, display_scale=0.2)
